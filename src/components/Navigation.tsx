...
const Navigation = ({ data }: NavigationProps) => {
  ...
  const handleScroll = useCallback(() => {
    ...
    const scrollPosition = scrollY + 120; // offset لتحديد السكشن بدقة
    ...
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-smooth ${isScrolled ? "bg-background/80 backdrop-blur-md border-b border-primary/20 shadow-card" : "bg-transparent"}`}>
      <LazyMotion features={domAnimation}>
        ...
        <div className="absolute top-0 left-0 w-full h-0.5 bg-transparent">
          <m.div className="h-full bg-gradient-primary shadow-glow" style={{ width: `${scrollProgress}%` }} transition={{ type: "spring", stiffness: 120, damping: 20 }} />
        </div>
              ...
              {navItems.map((item) => (
                <div key={item.id} className="relative group">
                  ...
                  >
                    ...
                    {/* Hover underline */}
                    <span className="absolute bottom-0 left-0 right-0 h-0.5 bg-gradient-primary rounded-full scale-x-0 group-hover:scale-x-100 transition-transform duration-300 origin-left"></span>
                ...
                {navItems.map((item) => (
                  <button
                    ...
                    className={`text-left px-4 py-3 text-sm font-medium transition-smooth hover:bg-primary/10 rounded-lg ${
                      activeSection === item.id ? "text-primary bg-primary/10" : "text-secondary"
                    }`}
                  >
                    {item.label}
                  </button>
